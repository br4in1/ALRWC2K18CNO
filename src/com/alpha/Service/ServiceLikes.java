...
public class ServiceLikes {
	...
	public Gallery listGallery3 = new Gallery();

	public Gallery existe(int id , int idphoto) {
		ConnectionRequest con = new ConnectionRequest();
		con.setUrl("http://127.0.0.1:8000/gallery/photo/exist/mobile/"+id+"/"+idphoto);

		con.addResponseListener(new ActionListener<NetworkEvent>() {
			@Override
			public void actionPerformed(NetworkEvent evt) {
				ServiceLikes ser = new ServiceLikes();
				listGallery3 = ser.getListGallery1(new String(con.getResponseData()));
			}
		});
		NetworkManager.getInstance().addToQueueAndWait(con);
		return listGallery3;
	...
	public Gallery getListGallery1(String json) {
		...
		try {
			...
			for (Map<String, Object> obj : list) {
				...
				listGallery.setExist((String) text);
			}
